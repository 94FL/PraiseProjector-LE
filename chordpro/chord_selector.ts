...
export class ChordSelector {
  ...
  /*
    private getCachedChordLayoutsByName(name: string) {
        return this.chordsCache[name] || this.chordsCache[name + "*"];
    }
    private updateNeck(initVariations: boolean) {

        this.tblNeck.innerHTML = "";
        if (this.parent_div.style.display === 'none' || this.selBaseNote.selectedIndex < 0 || this.selModifier.selectedIndex < 0) return;

        if (initVariations) {
            this.updateChordsCache();
            this.updateVariations();
        }

        const chordVariations = this.getCachedChordLayoutsByName(this.selModifier.value);
        const chord = chordVariations[Math.max(this.selVariation.selectedIndex, 0)];
        let minBund: number | null = null, maxBund: number | null = null;
        for (const pos of chord) if (pos !== null) {
            minBund = minBund !== null ? Math.min(pos, minBund) : pos;
            maxBund = maxBund !== null ? Math.max(pos, maxBund) : pos;
        }

        if (maxBund === null || minBund === null) return;

        if (maxBund < 5) { minBund = 1; maxBund = 5; }
        else if (maxBund - minBund < 4) {
            minBund = Math.max(minBund, 1);
            maxBund = minBund + 4;
        }
        minBund = 1; maxBund = 16;

        const addDot = (cell: HTMLElement, base: boolean) => {
            const dot = document.createElement("img");
            dot.src = base ? "images/red.png" : "images/dot.png";
            cell.appendChild(dot);
        };

        const guitarTuning = this.tuning;
        for (let i = -1; i < guitarTuning.length; ++i) {
            const row = this.tblNeck.insertRow(this.tblNeck.rows.length);
            const pos = (i >= 0) ? chord[i] : undefined;
            let cell = row.insertCell(-1);
            if (i >= 0) {
                cell.className = "headerCell";
                cell.innerText = baseNoteList[guitarTuning[i] % 12];
                cell.style.backgroundPositionY = "-" + i * this.fretHeight + "px";
                cell.style.width = this.headerWidth + "px";
                cell.style.height = this.fretHeight + "px";
            }
            cell = row.insertCell(-1);
            cell.className = (i >= 0) ? "fretCell" : "posCell";
            if (pos === null) cell.innerText = "X";
            else if (pos === 0) addDot(cell, i >= 0 && (guitarTuning[i] + pos) % 12 === this.selBaseNote.selectedIndex);
            cell.style.backgroundPositionX = "-" + this.headerWidth + "px";
            cell.style.backgroundPositionY = "-" + i * this.fretHeight + "px";
            cell.style.width = this.zeroWidth + "px";
            cell.style.height = this.fretHeight + "px";
            for (let bund = minBund; bund < maxBund; ++bund) {
                cell = row.insertCell(-1);
                cell.className = (i >= 0) ? "fretCell" : "posCell";
                if (pos === undefined && bund) cell.innerText = bund.toString();
                if (i >= 0) {
                    cell.style.backgroundPositionX = "-" + (this.headerWidth + this.zeroWidth + (bund - 1) * this.fretWidth) + "px";
                    cell.style.backgroundPositionY = "-" + i * this.fretHeight + "px";
                    cell.style.width = this.fretWidth + "px";
                    cell.style.height = this.fretHeight + "px";
                    if (pos === bund) addDot(cell, (guitarTuning[i] + pos) % 12 === this.selBaseNote.selectedIndex);
                }
            }
        }
    }
